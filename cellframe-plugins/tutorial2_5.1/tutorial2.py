...
def getwalletBalance(address):
    if len(address) != 104:
        ...
        data += "\n\n" 
        return data
    addr = ChainAddr.fromStr(address)
    net_id = addr.getNetId()
    chain_net =  Net.byId(net_id)
...
def http_handler(sh, httpCode):
    ...
    if wallet:
        for wallets in wallet:
            ...
            wallets = wallets[0]
            logIt.notice("Getting wallet info for " + wallets)
...
def init():
    ...
    HttpSimple.addProc(sc, "/wallet", 96000, http_handler)
    logIt.notice("Webserver started")
    return 0
